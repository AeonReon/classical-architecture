#!/usr/bin/env node
// check-videos.mjs — verify every curated topic video in assets/videos-data.js.
//
...
// still looks fine until someone presses play.
//
...
for (const [topic, v] of Object.entries(window.VIDEOS)) {
  const res = await check(v.id);
  if (!res.ok) {
    broken++;
    console.log(`  BROKEN  ${topic.padEnd(20)} ${v.id}  ${v.channel}\n          → ${res.reason}`);
    continue;
  }
  const drift = v.views ? Math.round(((res.views - v.views) / v.views) * 100) : 0;
  console.log(`  ok      ${topic.padEnd(20)} ${String(res.views).padStart(9)} views  ${drift >= 0 ? '+' : ''}${drift}%  ${v.channel}`);
